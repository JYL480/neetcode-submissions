...
class Solution:
    def trap(self, height: List[int]) -> int:
        ...
        for i, num in enumerate(height):
            ...
            a = after_list[i]
            # print(i,b, a)
            # if min(height[l], height[r]) - height[]

            # If either max l or max r  ==0 then continue
            if b ==0 or a ==0:
                continue
            else:
                # Do the coaculation
                height_cal = min(before_list[i], after_list[i]) - height[i]
                print(i, height_cal)
                if height_cal < 0:
                    continue
                count += height_cal
                print("count at this index",i ,count)
        return count
