...
class Solution:
    def twoSum(self, nums: List[int], target: int) -> List[int]:
        ...
        # Or you can do it in single pass, if not inside then store loh
        for num_index in range(len(nums)):
            
            complement = target - nums[num_index]

            if complement in num_hash and num_index != num_hash[complement]:
                return [num_hash[complement], num_index]

            # So if its not inside, then you will add it to the hash_map 
            num_hash[nums[num_index]] = num_index
            print(num_hash)
